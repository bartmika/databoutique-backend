...
func (impl AssistantStorerImpl) Create(ctx context.Context, u *Assistant) error {
	// DEVELOPER NOTES:
...
func (impl AssistantStorerImpl) generatePublicID(ctx context.Context, tenantID primitive.ObjectID) (uint64, error) {
	var publicID uint64
	latest, err := impl.GetLatestByTenantID(ctx, tenantID)
	if err != nil {
		impl.Logger.Error("database get latest assistant by tenant id error",
			slog.Any("error", err),
			slog.Any("tenant_id", tenantID))
		return 0, err
	}
	if latest == nil {
		impl.Logger.Debug("first assistant creation detected, setting publicID to value of 1",
			slog.Any("tenant_id", tenantID))
		publicID = 1
	} else {
		publicID = latest.PublicID + 1
		impl.Logger.Debug("system generated new assistant publicID",
