...
type AssistantPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
// AssistantPaginationListResult represents the paginated list results for
// the associate records.
type AssistantPaginationListResult struct {
...
func (impl AssistantStorerImpl) newPaginationFilter(f *AssistantPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl AssistantStorerImpl) newPaginationFilterBasedOnString(f *AssistantPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl AssistantStorerImpl) newPaginationFilterBasedOnTimestamp(f *AssistantPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl AssistantStorerImpl) newPaginationOptions(f *AssistantPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl AssistantStorerImpl) newPaginatorNextCursor(f *AssistantPaginationListFilter, results []*Assistant) (string, error) {
	var lastDatum *Assistant
