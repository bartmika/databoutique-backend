...
type ProgramPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
// ProgramPaginationListResult represents the paginated list results for
...
func (impl ProgramStorerImpl) newPaginationFilter(f *ProgramPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl ProgramStorerImpl) newPaginationFilterBasedOnString(f *ProgramPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl ProgramStorerImpl) newPaginationFilterBasedOnInt8(f *ProgramPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ProgramStorerImpl) newPaginationFilterBasedOnTimestamp(f *ProgramPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ProgramStorerImpl) newPaginationOptions(f *ProgramPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl ProgramStorerImpl) newPaginatorNextCursor(f *ProgramPaginationListFilter, results []*Program) (string, error) {
	var lastDatum *Program
