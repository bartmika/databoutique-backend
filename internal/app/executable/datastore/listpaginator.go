...
// ExecutablePaginationListResult represents the paginated list results for
...
func (impl ExecutableStorerImpl) newPaginationFilter(f *ExecutablePaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl ExecutableStorerImpl) newPaginationFilterBasedOnString(f *ExecutablePaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl ExecutableStorerImpl) newPaginationFilterBasedOnInt8(f *ExecutablePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ExecutableStorerImpl) newPaginationFilterBasedOnTimestamp(f *ExecutablePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ExecutableStorerImpl) newPaginationOptions(f *ExecutablePaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl ExecutableStorerImpl) newPaginatorNextCursor(f *ExecutablePaginationListFilter, results []*Executable) (string, error) {
	var lastDatum *Executable
