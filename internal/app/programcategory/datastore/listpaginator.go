...
type ProgramCategoryPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
// ProgramCategoryPaginationListResult represents the paginated list results for
// the associate records.
type ProgramCategoryPaginationListResult struct {
...
func (impl ProgramCategoryStorerImpl) newPaginationFilter(f *ProgramCategoryPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl ProgramCategoryStorerImpl) newPaginationFilterBasedOnString(f *ProgramCategoryPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl ProgramCategoryStorerImpl) newPaginationFilterBasedOnInt8(f *ProgramCategoryPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ProgramCategoryStorerImpl) newPaginationFilterBasedOnTimestamp(f *ProgramCategoryPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ProgramCategoryStorerImpl) newPaginationOptions(f *ProgramCategoryPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl ProgramCategoryStorerImpl) newPaginatorNextCursor(f *ProgramCategoryPaginationListFilter, results []*ProgramCategory) (string, error) {
	var lastDatum *ProgramCategory
