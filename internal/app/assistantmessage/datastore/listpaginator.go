...
type AssistantMessagePaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
	...
	AssistantThreadID primitive.ObjectID
	Status            int8
	SearchText        string
}

// AssistantMessagePaginationListResult represents the paginated list results for
// the associate records.
type AssistantMessagePaginationListResult struct {
...
func (impl AssistantMessageStorerImpl) newPaginationFilter(f *AssistantMessagePaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl AssistantMessageStorerImpl) newPaginationFilterBasedOnString(f *AssistantMessagePaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl AssistantMessageStorerImpl) newPaginationFilterBasedOnInt8(f *AssistantMessagePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl AssistantMessageStorerImpl) newPaginationFilterBasedOnTimestamp(f *AssistantMessagePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl AssistantMessageStorerImpl) newPaginationOptions(f *AssistantMessagePaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl AssistantMessageStorerImpl) newPaginatorNextCursor(f *AssistantMessagePaginationListFilter, results []*AssistantMessage) (string, error) {
	var lastDatum *AssistantMessage
