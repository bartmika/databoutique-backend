...
func (impl AssistantMessageStorerImpl) ListByFilter(ctx context.Context, f *AssistantMessagePaginationListFilter) (*AssistantMessagePaginationListResult, error) {
	...
	if !f.AssistantThreadID.IsZero() {
		filter["assistant_thread_id"] = f.AssistantThreadID
	}

	// if f.ExcludeArchived {
	// 	filter["status"] = bson.M{"$ne": AssistantMessageStatusArchived} // Do not list archived items! This code
	// }
	if f.Status != 0 {
	...
	return &AssistantMessagePaginationListResult{
		Results:     results,
