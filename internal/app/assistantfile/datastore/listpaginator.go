...
type AssistantFilePaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
// AssistantFilePaginationListResult represents the paginated list results for
// the customer records (meaning limited).
type AssistantFilePaginationListResult struct {
	Results     []*AssistantFile `json:"results"`
...
func (impl AssistantFileStorerImpl) newPaginationFilter(f *AssistantFilePaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl AssistantFileStorerImpl) newPaginationFilterBasedOnString(f *AssistantFilePaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl AssistantFileStorerImpl) newPaginationFilterBasedOnTimestamp(f *AssistantFilePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl AssistantFileStorerImpl) newPaginationOptions(f *AssistantFilePaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl AssistantFileStorerImpl) newPaginatorNextCursor(f *AssistantFilePaginationListFilter, results []*AssistantFile) (string, error) {
	var lastDatum *AssistantFile
...
// newPaginatorNextCursor will return the base64 encoded next cursor which works
// with our paginator.
func (impl AssistantFileStorerImpl) newPaginatorNextCursorForFull(f *AssistantFilePaginationListFilter, results []*AssistantFile) (string, error) {
	var lastDatum *AssistantFile
