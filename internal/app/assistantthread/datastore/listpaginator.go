...
type AssistantThreadPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
	...
	AssistantID primitive.ObjectID
	Status      int8
	SearchText  string
}

// AssistantThreadPaginationListResult represents the paginated list results for
// the associate records.
type AssistantThreadPaginationListResult struct {
...
func (impl AssistantThreadStorerImpl) newPaginationFilter(f *AssistantThreadPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl AssistantThreadStorerImpl) newPaginationFilterBasedOnString(f *AssistantThreadPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl AssistantThreadStorerImpl) newPaginationFilterBasedOnTimestamp(f *AssistantThreadPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl AssistantThreadStorerImpl) newPaginationOptions(f *AssistantThreadPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl AssistantThreadStorerImpl) newPaginatorNextCursor(f *AssistantThreadPaginationListFilter, results []*AssistantThread) (string, error) {
	var lastDatum *AssistantThread
