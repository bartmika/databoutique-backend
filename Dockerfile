...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o databoutique-backend .
...
COPY --from=build-env /app/databoutique-backend .
...
CMD [ "/databoutique-backend" ]

# BUILD
# (Run following from project root directory!)
# docker build -f Dockerfile -t LuchaComics/databoutique-backend:latest --platform linux/amd64 .

# EXECUTE
# docker tag LuchaComics/databoutique-backend:latest LuchaComics/databoutique-backend:latest

# UPLOAD
# docker push LuchaComics/databoutique-backend:latest
